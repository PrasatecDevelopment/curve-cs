...
namespace Prasatec.Raden
{
    internal static class ForeignKeyHelper
    {
        internal static T Convert<F, T>(F value)
        {
            ...
            T result;
            ulong realValue;
            if (typeof(F) == typeof(ulong) && ulong.TryParse(value?.ToString(), out realValue))
            {
                result = (T)Activator.CreateInstance(typeof(T), realValue);
            }
            else if (typeof(F) == typeof(IModel) && ulong.TryParse(((IModel)value)?.ID.ToString(), out realValue))
            {
                result = (T)Activator.CreateInstance(typeof(T), realValue);
            }
